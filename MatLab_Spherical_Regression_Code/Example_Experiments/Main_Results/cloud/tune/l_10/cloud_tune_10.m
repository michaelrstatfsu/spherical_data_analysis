...
for j=1:10
    filename=[filename '_' num2str(lambda(j)) '_' num2str(NN) '_' num2str(l) '.mat'];
...
lambda=(linspace(0,.1,10).^2);
for j=1:10
filename2= [filename '_' num2str(lambda(j)) '_60_' num2str(l) '.mat'];
load(filename2)
r(j)=rho(j)/length(x);
...
print(figtype,[  filename '_tuning_MSE' figext]);